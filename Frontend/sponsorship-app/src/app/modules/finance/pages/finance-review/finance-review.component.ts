...
export class FinanceReviewComponent {
  requests: SponsorshipRequest[] = [];
  userId: number = 0;
  isLoading = false;
  filterStatus: string = 'all';

  constructor(private router: Router, private authService: AuthService, private financeService: FinanceService) {
  ...
  loadMyRequests() {
    this.isLoading = true;
    this.financeService.getPendingRequests().subscribe({
      next: (requests) => {
